--RH EMU Friendly Key System for loadstring


return function(USER_KEY)
    -- Defensive: bail early if the wrapper forgot to pass a key
    if type(USER_KEY) ~= "string" or USER_KEY == "" then
        warn("[LOADER] No USER_KEY provided. The wrapper must call this with your key.")
        warn('[LOADER] Example: L("RH-XXXXX-XXXXX-XXXXX")')
        return
    end
    ...
    local function checkKeyStatus()
        ...
        if DEBUG then warn("[LOADER:DEBUG] URL: " .. url) end
    ...
    if not keyStatus then
        ...
        if DEBUG then warn("Send the [LOADER:DEBUG] lines above to support.") end
        return
    end
